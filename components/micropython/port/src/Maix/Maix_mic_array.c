...
// ============================================================================
// 【终极修复】：C90 严格声明 + 6通道 DC-Blocker + PCF 锐化掩膜
// ============================================================================
STATIC mp_obj_t Maix_mic_array_get_beam_audio(void) {
    ...
    float sum_energy = 0.0f, mic_energy = 0.0f;
    float frame_pcf, gain_factor, amplified;
    static float smoothed_gain = 1.0f;
    
    int i, m, target_idx;
    float sum, energy_i, ac_val, raw_val;
    mp_obj_t tuple[2];
    ...
    for (i = 0; i < FFT_N; i++) {
        ...
        for (m = 0; m < 6; m++) {
            ...
            if (target_idx >= 0 && target_idx < FFT_N) {
                raw_val = mic_raw[m][target_idx];
                // 核心修复：一阶高通滤波器剔除 I2S 直流偏置
