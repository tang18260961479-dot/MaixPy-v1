...
// ============================================================================
// 第二部分：时空级联 M-估计 测向管线 (已替换为真 IRLS 架构)
// ============================================================================
static float run_doa_pipeline(float mic_data[NUM_MICS][FFT_N]) {
    ...
    // 2. 前端 GCC-PHAT (获取 Meas_TDOA 与 Qual_Score)
    for (u = 0; u < NUM_MICS; u++) {
        for (v = u + 1; v < NUM_MICS; v++) {
            ...
            if (pair_conf[k].dist > d_max) d_max = pair_conf[k].dist; // 提取 D_max
            k++;
        }
    }
    ...
    for (p = 0; p < NUM_PAIRS; p++) {
        theo_tdoa = (pair_conf[p].dx * cos_B + pair_conf[p].dy * sin_B) / SOUND_SPEED;
        raw_residuals_B[p] = fabsf(Meas_TDOA[p] - theo_tdoa);
    }
    med_res_B = calculate_median(raw_residuals_B, NUM_PAIRS) * SOUND_SPEED;
    sigma_m_C = med_res_B * 1.5f; 
    if (sigma_m_C < 0.005f) sigma_m_C = 0.005f; // 【最新修复的死区边界】
    if (sigma_m_C > 0.10f)  sigma_m_C = 0.10f;
    sigma_t_C = sigma_m_C / SOUND_SPEED;

    for (p = 0; p < NUM_PAIRS; p++) {
        res_sq = raw_residuals_B[p] * raw_residuals_B[p];
        w_consist = expf(-res_sq / (2.0f * sigma_t_C * sigma_t_C));
        // Q_k * [(1-a)*w + a] * sqrt(d_k / D_max) 嵌套拓扑
        W_C[p] = Qual_Score[p] * ((1.0f - 0.2f) * w_consist + 0.2f) * sqrtf(pair_conf[p].dist / d_max);
    }

    for (theta = -180; theta < 180; theta++) {
        cost = 0.0f; cos_t = cosf(theta * DEG2RAD); sin_t = sinf(theta * DEG2RAD);
        for (p = 0; p < NUM_PAIRS; p++) {
            if (W_C[p] < 1e-4f) continue;
            theo_tdoa = (pair_conf[p].dx * cos_t + pair_conf[p].dy * sin_t) / SOUND_SPEED;
            err = theo_tdoa - Meas_TDOA[p];
            cost += W_C[p] * err * err;
        }
    ...
    for (iter = 0; iter < 8; iter++) {
        ...
        // 1. 实时更新残差
        for (p = 0; p < NUM_PAIRS; p++) {
            theo_tdoa = (pair_conf[p].dx * cos_irls + pair_conf[p].dy * sin_irls) / SOUND_SPEED;
            r_irls[p] = theo_tdoa - Meas_TDOA[p];
            raw_res_irls[p] = fabsf(r_irls[p]);
        }
        
        // 2. 动态提取 MAD 标尺
        med_res = calculate_median(raw_res_irls, NUM_PAIRS) * SOUND_SPEED;
        sigma_m = med_res * 1.5f;
        if (sigma_m < 0.005f) sigma_m = 0.005f; // 【底层防线】
        if (sigma_m > 0.10f)  sigma_m = 0.10f;
        sigma_t = sigma_m / SOUND_SPEED;

        // 3. 动态刷新复合权重与雅可比矩阵
        for (p = 0; p < NUM_PAIRS; p++) {
            if (Qual_Score[p] < 1e-4f) continue;
            
            res_sq = raw_res_irls[p] * raw_res_irls[p];
            w_consist = expf(-res_sq / (2.0f * sigma_t * sigma_t));
            w_diag = Qual_Score[p] * ((1.0f - 0.2f) * w_consist + 0.2f) * sqrtf(pair_conf[p].dist / d_max);
            
            J_p = DEG2RAD * (-pair_conf[p].dx * sin_irls + pair_conf[p].dy * cos_irls) / SOUND_SPEED;
            
            if (w_diag > 0.0f) {
                valid_count++;
                sum_WJr += w_diag * J_p * r_irls[p];
    ...
    raw_angle = fmodf(ang_irls + 180.0f, 360.0f);
...
// ============================================================================
// 第三部分：核心 API 与 【物理级 FD-PCF 频域束波成型】(未做任何修改)
// ============================================================================
