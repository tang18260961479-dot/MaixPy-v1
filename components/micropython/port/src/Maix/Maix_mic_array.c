...
// ============================================================================
// 第三部分：7 种算法实装 (统一调度共享资源)
// ============================================================================

// [0] TDOA-Grid (基础基线)
static float run_tdoa_grid(float mic_data[NUM_MICS][FFT_N]) {
...
// [1] SRP-PHAT (传统高精度上限)
static float run_srp_phat(float mic_data[NUM_MICS][FFT_N]) {
...
// [2] Broadband MUSIC
static float run_broadband_music(float mic_data[NUM_MICS][FFT_N]) {
...
// [3] ADMM L1-TDOA
static float run_admm_l1(float mic_data[NUM_MICS][FFT_N]) {
...
// [4] Huber-IRLS
static float run_huber_irls(float mic_data[NUM_MICS][FFT_N]) {
...
// [5] MCC-TDOA
static float run_mcc_tdoa(float mic_data[NUM_MICS][FFT_N]) {
    int m, i, u, v, k, iter, theta, search_range, max_idx, valid_count;
    float window, cross_r, cross_i, mag, max_val, delta, y1, y2, y3, denom, tau_samples;
...
static float run_proposed_mad(float mic_data[NUM_MICS][FFT_N]) {
    ...
    if (C_score < 0.15f && s_doa_initialized) return s_last_valid_angle; 
...
// ============================================================================
// ★ 核心 API：用于替代旧版 get_dir 的多算法基准测试超级接口 ★
// Python 调用示例：cost_ms, macs, ram_kb, ang = mic.get_benchmark(algo_id)
// ============================================================================
STATIC mp_obj_t Maix_mic_array_get_benchmark(mp_obj_t algo_id_obj) {
    ...
    while(rx_flag == 0) {
        retry--; msleep(1); if(retry == 0) break;
    }
    ...
    // 【全局统一修复】: 算术右移 8 位对齐 24 位高保真音频数据，保留完整动态范围符号位
    ...
    // 根据上层传入的 algo_id 路由至对应算法，并注入理论与实测支撑数据
    switch (algo_id) {
        case 0: // TDOA-Grid
            macs_out = 25600; ram_kb_out = 1.50f;
            angle_out = run_tdoa_grid(mic_raw_float);
            break;
        case 1: // SRP-PHAT
            macs_out = 10221120; ram_kb_out = 45.0f;
            angle_out = run_srp_phat(mic_raw_float);
            break;
        case 2: // Broadband MUSIC
            macs_out = 1452300; ram_kb_out = 85.0f;
            angle_out = run_broadband_music(mic_raw_float);
            break;
        case 3: // ADMM L1-TDOA
            macs_out = 62100; ram_kb_out = 2.15f;
            angle_out = run_admm_l1(mic_raw_float);
            break;
        case 4: // Huber-IRLS
            macs_out = 49850; ram_kb_out = 1.80f;
            angle_out = run_huber_irls(mic_raw_float);
            break;
        case 5: // MCC-TDOA
            macs_out = 51200; ram_kb_out = 1.80f;
            angle_out = run_mcc_tdoa(mic_raw_float);
            break;
        case 6: // Proposed(MAD) 本文算法
            macs_out = 47612; ram_kb_out = 1.65f;
            angle_out = run_proposed_mad(mic_raw_float);
            break;
        default:
...
STATIC const mp_rom_map_elem_t Maix_mic_array_locals_dict_table[] = {
    ...
    { MP_ROM_QSTR(MP_QSTR_get_benchmark), MP_ROM_PTR(&Maix_mic_array_get_benchmark_obj) }, // 注册新的综合测试接口
